...
import { ArrowLeft, TrendingUp, TrendingDown, Filter, Trash2 } from "lucide-react";
...
export default function TradeHistory() {
  ...
  const { data: credsData } = useQuery<any>({ queryKey: ["/api/credentials"], refetchInterval: 10000 });
  const [filterVersion, setFilterVersion] = useState<number | null>(null);
  const [clearConfirm, setClearConfirm] = useState(false);
  const [clearing, setClearing] = useState(false);

  const isConnected = credsData?.connected === true;
  const allTrades = (isConnected ? tradesData?.trades : []) ?? [];
  ...
  const handleClearHistory = async () => {
    ...
    try {
      ...
      if (res.ok) {
        qc.setQueryData(["/api/trades"], { trades: [] });
  ...
  return (
    <div className="min-h-screen">
      <nav className="glass-nav sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-5 h-14 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="text-xs text-[#7A7468]">{allTrades.length} trades</div>
            {isConnected && allTrades.length > 0 && !clearConfirm && (
              <button
                onClick={() => setClearConfirm(true)}
                className="glass-btn flex items-center gap-1.5 px-3 py-1.5 text-xs text-red-400/70 hover:text-red-400 border border-red-400/20 hover:border-red-400/40 rounded-lg transition-all"
              >
                <Trash2 size={11} />
                Clear History
              </button>
            ...
            {clearConfirm && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-red-400">Clear all trades?</span>
                <button
                  onClick={handleClearHistory}
                  disabled={clearing}
                  className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-red-500/20 border border-red-500/40 text-red-400 hover:bg-red-500/30 transition-all disabled:opacity-50"
                >
                  {clearing ? "Clearing..." : "Yes, clear"}
                </button>
                <button
                  onClick={() => setClearConfirm(false)}
                  className="px-3 py-1.5 text-xs rounded-lg glass-btn text-[#F4EFE6]/50 hover:text-[#F4EFE6]/80"
                >
                  Cancel
                </button>
              </div>
      ...
      <main className="max-w-5xl mx-auto px-5 py-6 space-y-4">
        ...
        {!isConnected && (
          <div className="glass p-4 border border-yellow-400/30 bg-yellow-400/5 rounded-xl">
            <div className="flex items-center gap-2 text-yellow-400 text-sm font-semibold mb-1">
              ⚠ Not connected
            </div>
            <div className="text-xs text-[#F4EFE6]/50">
              No Kalshi API credentials are configured. Trade history is only shown when connected.{" "}
              <Link href="/settings"><span className="text-[#C9A84C] cursor-pointer underline">Go to Settings</span></Link> to add your credentials.
            </div>
          </div>
        )}
